import {
  ...
  // Save metafields to the selected collection
  export const action = async ({ request }) => {
    const { admin } = await authenticate.admin(request);
    const formData = await request.formData();
  
    const triggerCollectionId = formData.get("triggerCollectionId");
  ...
  export default function OfferSelectionPage() {
    ...
    const [triggerCollection, setTriggerCollection] = useState(null);
    const [offerProducts, setOfferProducts] = useState([]);
    const [displayStyle, setDisplayStyle] = useState("toggle");
  
    const handleTriggerSelect = (collection) => {
      setTriggerCollection(collection);
    ...
    return (
      <Page title="Create Offer">
        <Layout>
          ...
          <Layout.Section>
            <Card title="Trigger Collection" sectioned>
              <Text>Select trigger collection</Text>
              <BlockStack gap="200">
                <Button onClick={() => setTriggerModalOpen(true)}>Select Trigger Collection</Button>
                {triggerCollection && (
                  <BlockStack gap="100">
                    <Thumbnail
                      source={triggerCollection.node.image?.originalSrc || ""}
                      alt={triggerCollection.node.image?.altText || ""}
                    />
                    <Text>{triggerCollection.node.title}</Text>
                  </BlockStack>
                )}
              </BlockStack>
            </Card>
            ...
            <Card sectioned>
              <Form method="post">
                <input
                  ...
                  value={triggerCollection?.node.id || ""}
                />
                <input
                  type="hidden"
                ...
                />
                <br />
                <Button submit primary disabled={!triggerCollection || offerProducts.length === 0}>
                  Save Offer
                </Button>
              </Form>
            </Card>
        ...
        {/* Trigger Collection Modal */}
        <Modal
          open={triggerModalOpen}
          onClose={() => setTriggerModalOpen(false)}
          title="Select Trigger Collection"
          large
        >
          <Modal.Section>
            <ResourceList
